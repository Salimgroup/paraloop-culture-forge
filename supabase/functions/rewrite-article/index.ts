...
import { rewriteArticleSchema, parseJsonBody } from "../_shared/validation.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isValidHmac) {
      console.error('Invalid HMAC signature for rewrite-article');
      return unauthorizedResponse('Invalid or missing HMAC signature');
    }
    
    // Parse and validate input with higher limit for extracted text
    let body: unknown;
    try {
      body = await parseJsonBody(req, 100); // 100KB max for article content
    } catch (error) {
      return badRequestResponse(error instanceof Error ? error.message : 'Invalid request body');
    }
    
    const validation = rewriteArticleSchema.safeParse(body);
    if (!validation.success) {
      return badRequestResponse('Validation failed', validation.error.flatten());
    }
    
    const { article, extractedText } = validation.data;
